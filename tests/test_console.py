...
class TestHBNBCommand_help(unittest.TestCase):
    ...
    def test_help_create(self):
        h = ("Creates a new instance of BaseModel, saves it (to the JSON file)"
             "\n        and prints the id")
        with patch("sys.stdout", new=StringIO()) as output:
    ...
    def test_help_all(self):
        h = ("Prints all string representation of all instances based or not "
             "on\n        the class name")
        with patch("sys.stdout", new=StringIO()) as output:
...
class TestHBNBCommand_update(unittest.TestCase):
    ...
    def test_update_valid_int_attr_space_notation(self):
        with patch("sys.stdout", new=StringIO()) as output:
            HBNBCommand().onecmd("create Place")
            testId = output.getvalue().strip()
        testCmd = "update Place {} number_rooms 3".format(testId)
        self.assertFalse(HBNBCommand().onecmd(testCmd))
        test_dict = storage.all()["Place.{}".format(testId)].__dict__
        self.assertEqual(3, test_dict["number_rooms"])
